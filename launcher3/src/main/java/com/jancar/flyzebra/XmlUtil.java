...
public class XmlUtil<T> {
     ...
     * @param elements    //d（这两行标红，是因为这两个是java中字段和xml文件中的字段对应）队医节点集合一一对应字段集合
     * @param itemElement 每一项的节点标签
     * @return
     */
//静态方法中加入泛型，需要申明<T>，如果不是对泛型不是很熟悉，可以先用Object代替，然后再换回来
    public static <T> List<T> parse(InputStream is, Class<T> clazz, String[] fields, String[] elements, String itemElement){
        return parse(is,clazz,Arrays.asList(fields),Arrays.asList(elements),itemElement);
    }
    public static <T> List<T> parse(InputStream is, Class<T> clazz, List<String> fields, List<String> elements, String itemElement) {
        ...
        try {
            ...
            FlyLog.e("解析XML异常：" + e.getMessage());
        }
        return list;
    }


    public static <T> List<T> parse(XmlPullParser xmlPullParser, Class<T> clazz, List<String> fields, List<String> elements, String itemElement) {
        FlyLog.d("开始解析XML.");
        List<T> list = new ArrayList<T>();
        try {
            int event = xmlPullParser.getEventType();
